...
# Arrays
This is how to find the middle index of an array with an odd number of elements: ```arrayExample[Math.floor(arrayExample.length / 2)]```.  
To finf the median of an even index (it would be the two most middle numbers): First Number:```arrayExample[arrayExample.length/2]```, Second Number: ```arrayExample[(arrayExample/2) - 1]```, then calculate the median with the ```getMean()``` function like: ```getMean([firstNumber, secondNumber])```.
# Sorting Algorithms 
